...
export function indentRange(state: EditorState, from: number, to: number) {
  ...
  const context = new IndentContext(state, { overrideIndentation: (start) => updated[start] ?? -1 });
...
export class IndentContext {
  ...
  column(pos: number, bias: -1 | 1 = 1) {
    ...
    const override = this.options.overrideIndentation ? this.options.overrideIndentation(from) : -1;
...
function indentFrom(node: SyntaxNode | null, pos: number, base: IndentContext) {
  for (; node; node = node.parent) {
    const strategy = indentStrategy(node);
