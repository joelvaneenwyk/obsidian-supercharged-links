...
import { IndentContext, getIndentUnit, indentService } from './indent';
import { Language, ParseContext, defineLanguageFacet, languageDataProp, syntaxTree } from './language';
...
export class StreamLanguage<State> extends Language {
  ...
  private constructor(parser: StreamParser<State>) {
    const create = (streamLanguageInstance: StreamLanguage<State>) => {
      // Create a new parser instance to parse a new input.
      return new (class extends Parser {
        // Create a new Parse object to hold the state of the parser.
        createParse(input: Input, fragments: readonly TreeFragment[], ranges: readonly { from: number; to: number }[]) {
          return new Parse(streamLanguageInstance, input, fragments, ranges);
        }
      })();
    };
    ...
    super(
      data,
      // @ts-ignore The 'this' is an unusual pattern since we haven't called the constructor yet
      create(this),
      [indentService.of((cx, pos) => this.getIndent(cx, pos))],
      parser.name
    );
    this.topNode = docID(data);
    this.streamParser = fullParser(parser);
    ...
    while (statePos < pos) {
      ...
      if (line.length) {
        ...
        const lookahead = (n: number) =>
          n + line.number > cx.state.doc.lines ? '' : cx.state.doc.line(n + line.number).text;
...
function findStartInFragments<State>(
  ...
  for (const f of fragments) {
    ...
    const found = from <= startPos && to > startPos && findState(lang, f.tree, 0 - f.offset, startPos, to);
    let tree;
...
class Parse<State> implements PartialParse {
  ...
  advance() {
    ...
    const viewportEnd = (context?.viewport?.to ?? 0) + overBuffer;
  ...
  parseLine(context: ParseContext | null) {
        ...
        if (token) {
          ...
          if (cached) {
            ...
            tokens = tokens.filter((t) => {
              ...
              const lineClass = t.match(/^line-(background-)?(.*)$/);
              if (lineClass) {
                lineClear.push(lineClass[2]);
                return false;
              }
              return true;
            });
            cls = tokens.sort().join(' ');
            tokenCache[token] = [cls, lineClear];
          }
          if (lineClear.length > 0) {
            for (const cls of lineClear) {
              lineClasses.add(cls);
            }
          }
    ...
    if (lineClasses.size > 0) {
      offset = this.emitToken(
        tokenID(Array.from(lineClasses).sort().join(' '), true),
        this.parsedPos,
    ...
    for (const { from, to } of view.visibleRanges) {
      this.tree.iterate({
        ...
        enter: ({ type, from, to }) => {
          ...
          if (lineStyle) {
            const lineDeco =
              this.lineCache[lineStyle] ||
              (this.lineCache[lineStyle] = Decoration.line({ attributes: { class: lineStyle } }));
...
export const lineHighlighter = Prec.lowest(
  ViewPlugin.define((view) => new LineHighlighter(view), {
