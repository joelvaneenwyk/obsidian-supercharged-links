    ...
    const explore = (tree: Tree, from: number) => {
      ...
      if (mount) {
        if (mount.tree.prop(languageDataProp) == this.data) {
          if (mount.overlay) for (const r of mount.overlay) result.push({ from: r.from + from, to: r.to + from });
...
class LanguageState {
  ...
  static init(state: EditorState) {
    ...
    const parseState = ParseContext.create(state.facet(language)!.parser, state, { from: 0, to: vpTo });
...
export const language = Facet.define<Language, Language | null>({
  ...
  enables: (language) => [
    ...
    EditorView.contentAttributes.compute([language], (state) => {
      const lang = state.facet(language);
      return lang && lang.name ? { 'data-language': lang.name } : ({} as NonNullable<unknown>);
    })
  ]
});
