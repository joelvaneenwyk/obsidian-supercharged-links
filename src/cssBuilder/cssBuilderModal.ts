...
export function displayText(link: CSSLink, settings: SuperchargedLinksSettings): string {
  if (link.type === 'tag') {
    ...
    return `<span class="data-link-icon data-link-text data-link-icon-after" data-link-${link.name}="${
      link.value
    }">Note</span> has attribute <b>${link.name}</b> ${matchPreview[link.match]} <b>${link.value}</b>.`;
  }
  if (!link.value) {
    return '<b>Please choose a path.</b>';
  }
  return `The path of the <span class="data-link-icon data-link-text data-link-icon-after" data-link-path="${
    link.value
  }">note</span> ${matchPreviewPath[link.match]} <b>${link.value}</b>`;
}
...
class CSSBuilderModal extends Modal {
  ...
  constructor(plugin: SuperchargedLinks, saveCallback: (cssLink: CSSLink) => void, cssLink?: CSSLink) {
    super(plugin.app);
  ...
  onOpen() {
    ...
    new Setting(this.contentEl)
      ...
      .setDesc(
        'Attributes selects YAML and DataView attributes' +
      ...
      .addDropdown((dc: DropdownComponent) => {
        Object.keys(selectorType).forEach((key: string) => {
          const type = key as SelectorTypes;
          dc.addOption(type, selectorType[type]);
          if (type === this.cssLink.type) {
            dc.setValue(type);
          }
        });
        dc.onChange((key: string) => {
          const type = key as SelectorTypes;
          cssLink.type = type;
    ...
    const matchingType = new Setting(this.contentEl)
      ...
      .addDropdown((dc) => {
        Object.keys(matchTypes).forEach((match: string) => {
          const key: MatchTypes = match as MatchTypes;
