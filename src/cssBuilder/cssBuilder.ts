...
export async function buildCSS(selectors: CSSLink[], plugin: SuperchargedLinks) {
  ...
  selectors.forEach((selector) => {
    ...
    if (selector.type === 'attribute') {
      cssSelector = `[data-link-${selector.name}${matchSign[selector.match]}="${selector.value}" ${
        selector.matchCaseSensitive ? '' : ' i'
      }]`;
    } else if (selector.type === 'tag') {
      cssSelector = `[data-link-tags*="${selector.value}" i]`;
    } else {
      cssSelector = `[data-link-path${matchSign[selector.match]}="${selector.value}" ${
        selector.matchCaseSensitive ? '' : 'i'
      }]`;
    }
  ...
  instructions.push(...['/* @settings', 'name: Supercharged Links', 'id: supercharged-links', 'settings:']);
