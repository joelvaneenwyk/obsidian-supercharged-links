...
import { App, Debouncer, PluginSettingTab, Setting, debounce } from 'obsidian';
...
export default class SuperchargedLinksSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    /* Managing extra attributes for a.internal-link */
...
Styling can be done using the Style Settings plugin.
 ...
 <li>Go to the Style Settings tab and style your links!</li>
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'If true, this will automatically activate the generated CSS snippet "supercharged-links-gen.css". ' +
  ...
  drawSelectors(div: HTMLElement) {
    ...
    selectors.forEach((selector, i) => {
      const s = new Setting(div)
        ...
        .addButton((button) => {
          button.onClick(() => {
            const formModal = new CSSBuilderModal(
              this.plugin,
              (newSelector) => {
