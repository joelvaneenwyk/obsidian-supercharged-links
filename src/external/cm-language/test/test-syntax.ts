...
const lines = `const {readFile} = require("fs");
...
describe("ParseContext", () => {
    it("can parse a document", () => {
        const cx = pContext(Text.of(["let x = 10"]));
        cx.work(1e8);
    ...
    it("can parse incrementally", () => {
        ...
        const change = ChangeSet.of({from: 0, to: 5, insert: "let"}, doc.length);
        const newDoc = change.apply(doc);
...
describe("Indentation", () => {
    it("tracks indent units", () => {
        ...
        const s1 = EditorState.create({extensions: indentUnit.of("   ")});
        ist(getIndentUnit(s1), 3);
        ist(indentString(s1, 4), "    ");
        const s2 = EditorState.create({extensions: [indentUnit.of("\t"), EditorState.tabSize.of(8)]});
        ist(getIndentUnit(s2), 8);
        ist(indentString(s2, 16), "\t\t");
        const s3 = EditorState.create({extensions: indentUnit.of("　")});
